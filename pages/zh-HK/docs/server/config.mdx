...
配置文件默認路徑為:`./etc/archnet.yaml`，可通過啟動參數 `--config` 指定配置文件路徑。
...
Logger: # 日誌配置
  FilePath: # 日誌文件路徑, 默認: ./archnet.log
...
Administer:
  Email: # 後台登錄郵箱, 默認: admin@archnets.com
...
| 環境變量     | 配置項    | 示例                                        |
| ------------ | --------- | :------------------------------------------ |
| ARCHNET_DB    | MySQL配置 | root:password\@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS | Redis配置 | redis\://localhost:6379"                    |
