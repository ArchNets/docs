...
ArchNet podporuje webovou inicializaci, což zjednodušuje proces ruční konfigurace.

### Krok za krokem k inicializaci

1. **Vytvoření nezbytného konfiguračního souboru**: Nejprve ručně vytvořte a nakonfigurujte soubor `/app/etc/archnet.yaml` pro další konfiguraci.
...
5. **Klikněte na tlačítko inicializace**: Po dokončení konfigurace klikněte na tlačítko „Inicializovat“, aplikace automaticky uloží konfiguraci a restartuje se.
   ...
   > **Poznámka**: ArchNet ve výchozím nastavení používá port **8080**, ujistěte se, že je tento port přístupný, a v případě potřeby upravte nastavení firewallu.

## Nasazení pomocí Dockeru

ArchNet ve výchozím nastavení používá port **8080**, ujistěte se, že je tento port přístupný, a v případě potřeby upravte nastavení firewallu.

Pokud nepoužíváte webovou inicializaci, můžete se přihlásit pomocí výchozího administrátorského účtu:

- **Uživatelské jméno**: `admin@archnets.com`
- **Heslo**: `password`
...
Před zahájením nasazení pomocí Dockeru nejprve ručně vytvořte a nakonfigurujte `/app/etc/archnet.yaml`, abyste zajistili správný chod systému. Pro podrobné příklady konfiguračních položek se podívejte na část [Příklady konfiguračních položek](#příklady-konfiguračních-položek) v dokumentaci.
...
Před zahájením nasazení pomocí Docker Compose nejprve ručně vytvořte a nakonfigurujte `/app/etc/archnet.yaml`, abyste zajistili správný chod systému. Pro podrobné příklady konfiguračních položek se podívejte na část [Příklady konfiguračních položek](#příklady-konfiguračních-položek) v dokumentaci.
...
Níže je příklad konfiguračních položek pro `archnet.yaml`:
...
Logger:
  FilePath: ./archnet.log # Cesta k souboru s logy
  MaxSize: 50
  MaxBackup: 3
  MaxAge: 30
  Compress: true
...
Administer:
  ...
  Email: admin@archnets.com # E-mailová adresa administrátora
```
