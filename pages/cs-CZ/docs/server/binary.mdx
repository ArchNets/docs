...
V příkazovém rozhraní MariaDB proveďte následující příkaz pro vytvoření nové databáze archnet_db a nastavte znakový kód na utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Dále vytvořte místního uživatele databáze a omezte jeho oprávnění na nově vytvořenou databázi, použijte archnet jako uživatelské jméno a archnet-password jako heslo uživatele.
...
Stahovací adresa: `https://github.com/ArchNets/archnet/releases`
...
- Upravte konfigurační soubor archnet.yaml
...
Logger: # Konfigurace protokolování
  FilePath: ./archnet.log # Cesta k protokolovacímu souboru, výchozí: ./archnet.log
...
MySQL:
  ...
  Username: archnet # Uživatelské jméno MySQL, povinné
  Password: archnet-password # Heslo MySQL, povinné
  Dbname: archnet_db # Název databáze MySQL, povinné
...
Administer:
  Email: admin@archnets.com # E-mail pro přihlášení do administrace, výchozí: admin@archnets.com
...
2. Služba systemd:
   ...
   - Název služby: archnet
   - Konfigurační soubor služby: /etc/systemd/system/archnet.service
   - Příkaz pro spuštění služby: systemctl start archnet
   - Příkaz pro zastavení služby: systemctl stop archnet
   - Příkaz pro restartování služby: systemctl restart archnet
   - Příkaz pro kontrolu stavu služby: systemctl status archnet
   - Automatické spuštění služby při startu: systemctl enable archnet
...
4. Protokol služby: protokol služby se ve výchozím nastavení ukládá do souboru /usr/local/bin/archnet.log

5. Pro zobrazení protokolu služby můžete použít `journalctl -u archnet -f`

6. Pokud je konfigurační soubor prázdný nebo neexistuje, služba se spustí s výchozí konfigurací, cesta k konfiguračnímu souboru je: `./etc/archnet.yaml`,
