...
Výchozí cesta k konfiguračnímu souboru je: `./etc/archnet.yaml`, lze ji specifikovat pomocí spouštěcího parametru `--config`.
...
Logger: # Konfigurace protokolování
  FilePath: # Cesta k protokolovacímu souboru, výchozí: ./archnet.log
...
Administer:
  Email: # E-mail pro přihlášení do administrace, výchozí: admin@archnets.com
...
| ------------------ | -------------------- | :----------------------------------------- |
| ARCHNET_DB          | MySQL konfigurace    | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS       | Redis konfigurace    | redis://localhost:6379                     |
