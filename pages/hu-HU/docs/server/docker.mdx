...
A ArchNet támogatja a webes alapú inicializálást, amely leegyszerűsíti a manuális konfigurációs folyamatot.

### Inicializálási lépések

1. **Készítse el a szükséges konfigurációs fájlokat**: Először manuálisan hozzon létre és konfiguráljon egy `/app/etc/archnet.yaml` fájlt a további konfigurációhoz.
...
5. **Kattintson az inicializáló gombra**: A konfiguráció befejezése után kattintson az "Inicializálás" gombra, az alkalmazás automatikusan elmenti a konfigurációt és újraindul.
   ...
   > **Megjegyzés**: A ArchNet alapértelmezés szerint a **8080** portot használja, kérjük, győződjön meg arról, hogy ez a port elérhető, szükség esetén állítsa be a tűzfal beállításait.

## Docker használata a telepítéshez

A ArchNet alapértelmezés szerint a **8080** portot használja, kérjük, győződjön meg arról, hogy ez a port elérhető, szükség esetén állítsa be a tűzfal beállításait.

Ha nem használja a webes inicializálást, bejelentkezhet az alapértelmezett adminisztrátori fiókkal:

- **Felhasználónév**: `admin@archnets.com`
- **Jelszó**: `password`
...
A Docker telepítése előtt először manuálisan hozzon létre és konfiguráljon egy `/app/etc/archnet.yaml` fájlt, hogy a rendszer megfelelően működjön. A részletes konfigurációs példákért kérjük, nézze meg a dokumentáció [konfigurációs példák](#konfigurációs-példák) szakaszát.
...
A Docker Compose telepítése előtt először manuálisan hozzon létre és konfiguráljon egy `/app/etc/archnet.yaml` fájlt, hogy a rendszer megfelelően működjön. A részletes konfigurációs példákért kérjük, nézze meg a dokumentáció [konfigurációs példák](#konfigurációs-példák) szakaszát.
...
Az alábbiakban bemutatjuk a `archnet.yaml` konfigurációs példáit:
...
Logger:
  FilePath: ./archnet.log # Naplófájl elérési útja
  MaxSize: 50
  MaxBackup: 3
  MaxAge: 30
  Compress: true
...
Administer:
  ...
  Email: admin@archnets.com # Adminisztrátori e-mail cím
```
