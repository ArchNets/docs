...
A konfigurációs fájl alapértelmezett elérési útja: `./etc/archnet.yaml`, amelyet a `--config` indítási paraméterrel lehet megadni.
...
Logger: # Naplózási konfiguráció
  FilePath: # Naplófájl elérési útja, alapértelmezett: ./archnet.log
...
Adminisztráció:
  Email: # Háttérbe való bejelentkezés e-mail címe, alapértelmezett: admin@archnets.com
...
| ------------------ | ------------------ | :------------------------------------------ |
| ARCHNET_DB          | MySQL konfiguráció | root:password\@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS       | Redis konfiguráció | redis\://localhost:6379                     |
