...
W interfejsie wiersza poleceń MariaDB wykonaj następujące polecenie, aby utworzyć nową bazę danych archnet_db i ustawić zestaw znaków na utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Następnie utwórz lokalnego użytkownika bazy danych i ogranicz jego uprawnienia do nowo utworzonej bazy danych, używając archnet jako nazwy użytkownika i archnet-password jako hasła użytkownika.
...
Adres pobierania: `https://github.com/ArchNets/archnet/releases`
...
- Edytuj plik konfiguracyjny archnet.yaml
...
Logger: # Konfiguracja logowania
  FilePath: ./archnet.log # Ścieżka do pliku logu, domyślnie: ./archnet.log
...
MySQL:
  ...
  Username: archnet # Nazwa użytkownika MySQL, wymagane
  Password: archnet-password # Hasło MySQL, wymagane
  Dbname: archnet_db # Nazwa bazy danych MySQL, wymagane
...
Administer:
  Email: admin@archnets.com # E-mail do logowania w panelu administracyjnym, domyślnie: admin@archnets.com
...
2. Usługa systemd:
   ...
   - Nazwa usługi: archnet
   - Plik konfiguracyjny usługi: /etc/systemd/system/archnet.service
   - Polecenie uruchamiające usługę: systemctl start archnet
   - Polecenie zatrzymujące usługę: systemctl stop archnet
   - Polecenie restartujące usługę: systemctl restart archnet
   - Polecenie sprawdzające status usługi: systemctl status archnet
   - Automatyczne uruchamianie usługi przy starcie systemu: systemctl enable archnet
...
4. Logi usługi: logi usługi domyślnie są zapisywane w pliku /usr/local/bin/archnet.log

5. Można przeglądać logi usługi za pomocą `journalctl -u archnet -f`

6. Gdy plik konfiguracyjny jest pusty lub nie istnieje, usługa uruchomi się z domyślną konfiguracją, a ścieżka pliku konfiguracyjnego to: `./etc/archnet.yaml`,
