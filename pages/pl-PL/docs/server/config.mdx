...
Domyślna ścieżka do pliku konfiguracyjnego to: `./etc/archnet.yaml`, można ją określić za pomocą parametru uruchamiania `--config`.
...
Logger: # Konfiguracja logowania
  FilePath: # Ścieżka do pliku logów, domyślnie: ./archnet.log
...
Administer:
  Email: # E-mail do logowania w panelu administracyjnym, domyślnie: admin@archnets.com
...
| -------------------- | ---------------------- | :----------------------------------------- |
| ARCHNET_DB            | Konfiguracja MySQL     | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS         | Konfiguracja Redis     | redis://localhost:6379                     |
