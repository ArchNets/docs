...
MariaDB:n komentoriviliittymässä suorita seuraava komento luodaksesi uusi tietokanta archnet_db ja asettaaksesi merkistön utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Seuraavaksi luo paikallinen tietokantakäyttäjä ja rajoita tämän käyttäjän oikeudet juuri luotuun tietokantaan, käyttäen archnet -käyttäjänimeä ja archnet-password -salasanaa.
...
Latausosoite: `https://github.com/ArchNets/archnet/releases`
...
- Muokkaa archnet.yaml -konfiguraatiotiedostoa
...
Logger: # Lokituskonfiguraatio
  FilePath: ./archnet.log # Lokitiedoston polku, oletus: ./archnet.log
...
MySQL:
  ...
  Username: archnet # MySQL-käyttäjänimi, pakollinen
  Password: archnet-password # MySQL-salasana, pakollinen
  Dbname: archnet_db # MySQL-tietokannan nimi, pakollinen
...
Administer:
  Email: admin@archnets.com # Taustakirjautumisen sähköposti, oletus: admin@archnets.com
...
2. systemd-palvelu:
   ...
   - Palvelun nimi: archnet
   - Palvelun konfiguraatiotiedosto: /etc/systemd/system/archnet.service
   - Palvelun käynnistyskomento: systemctl start archnet
   - Palvelun pysäytyskomento: systemctl stop archnet
   - Palvelun uudelleenkäynnistyskomento: systemctl restart archnet
   - Palvelun tilakomento: systemctl status archnet
   - Palvelun automaattinen käynnistys: systemctl enable archnet
...
4. Palvelulokit: Palvelulokit tulostuvat oletuksena /usr/local/bin/archnet.log -tiedostoon.

5. Voit tarkastella palvelulokeja komennolla `journalctl -u archnet -f`.

6. Kun konfiguraatiotiedosto on tyhjää tai ei ole olemassa, palvelu käynnistyy oletusasetuksilla, konfiguraatiotiedoston polku on: `./etc/archnet.yaml`,
