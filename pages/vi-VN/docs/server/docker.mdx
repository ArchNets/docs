...
ArchNet hỗ trợ khởi tạo dựa trên Web, giúp đơn giản hóa quá trình cấu hình thủ công.

### Các bước khởi tạo

1. **Tạo tệp cấu hình cần thiết**: Đầu tiên, hãy tạo và cấu hình tệp `/app/etc/archnet.yaml` một cách thủ công để thực hiện các cấu hình tiếp theo.
...
5. **Nhấn nút khởi tạo**: Sau khi hoàn tất cấu hình, nhấn nút “Khởi tạo”, ứng dụng sẽ tự động lưu cấu hình và khởi động lại.
   ...
   > **Lưu ý**: ArchNet mặc định sử dụng cổng **8080**, hãy đảm bảo cổng này có thể truy cập, điều chỉnh cài đặt tường lửa nếu cần.

## Sử dụng Docker để triển khai

ArchNet mặc định sử dụng cổng **8080**, hãy đảm bảo cổng này có thể truy cập, điều chỉnh cài đặt tường lửa nếu cần.

Nếu không sử dụng khởi tạo qua Web, bạn có thể đăng nhập bằng tài khoản quản trị viên mặc định:

- **Tên người dùng**: `admin@archnets.com`
- **Mật khẩu**: `password`
...
Trước khi bắt đầu triển khai Docker, hãy tạo và cấu hình tệp `/app/etc/archnet.yaml` một cách thủ công để đảm bảo hệ thống hoạt động bình thường. Để biết thêm chi tiết về các mục cấu hình, hãy tham khảo phần [Ví dụ về các mục cấu hình](#ví-dụ-về-các-mục-cấu-hình) trong tài liệu.
...
Trước khi bắt đầu triển khai Docker Compose, hãy tạo và cấu hình tệp `/app/etc/archnet.yaml` một cách thủ công để đảm bảo hệ thống hoạt động bình thường. Để biết thêm chi tiết về các mục cấu hình, hãy tham khảo phần [Ví dụ về các mục cấu hình](#ví-dụ-về-các-mục-cấu-hình) trong tài liệu.
...
Dưới đây là ví dụ về các mục cấu hình trong `archnet.yaml`:
...
Logger:
  FilePath: ./archnet.log # Đường dẫn tệp nhật ký
  MaxSize: 50
  MaxBackup: 3
  MaxAge: 30
  Compress: true
...
Administer:
  ...
  Email: admin@archnets.com # Địa chỉ email quản trị viên
```
