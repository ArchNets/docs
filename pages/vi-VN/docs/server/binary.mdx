...
Trong giao diện dòng lệnh của MariaDB, thực hiện lệnh sau để tạo một cơ sở dữ liệu mới `archnet_db` và đặt bộ ký tự là `utf8mb4_unicode_ci`

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Tiếp theo, tạo một người dùng cơ sở dữ liệu cục bộ và giới hạn quyền của người dùng đó cho cơ sở dữ liệu mới tạo, sử dụng `archnet` làm tên người dùng và `archnet-password` làm mật khẩu người dùng.
...
Địa chỉ tải xuống: `https://github.com/ArchNets/archnet/releases`
...
- Chỉnh sửa tệp cấu hình `archnet.yaml`
...
Logger: # Cấu hình nhật ký
  FilePath: ./archnet.log # Đường dẫn tệp nhật ký, mặc định: ./archnet.log
...
MySQL:
  ...
  Username: archnet # Tên người dùng MySQL, bắt buộc
  Password: archnet-password # Mật khẩu MySQL, bắt buộc
  Dbname: archnet_db # Tên cơ sở dữ liệu MySQL, bắt buộc
...
Administer:
  Email: admin@archnets.com # Email đăng nhập quản trị, mặc định: admin@archnets.com
...
2. Dịch vụ systemd:
   ...
   - Tên dịch vụ: archnet
   - Tệp cấu hình dịch vụ: `/etc/systemd/system/archnet.service`
   - Lệnh khởi động dịch vụ: `systemctl start archnet`
   - Lệnh dừng dịch vụ: `systemctl stop archnet`
   - Lệnh khởi động lại dịch vụ: `systemctl restart archnet`
   - Lệnh kiểm tra trạng thái dịch vụ: `systemctl status archnet`
   - Tự khởi động khi khởi động hệ thống: `systemctl enable archnet`
...
4. Nhật ký dịch vụ: Nhật ký dịch vụ mặc định được xuất ra tệp `/usr/local/bin/archnet.log`

5. Có thể xem nhật ký dịch vụ bằng lệnh `journalctl -u archnet -f`

6. Khi tệp cấu hình trống hoặc không tồn tại, dịch vụ sẽ sử dụng cấu hình mặc định để khởi động, đường dẫn tệp cấu hình là: `./etc/archnet.yaml`,
