...
```shell
bash <(curl -Ls https://raw.githubusercontent.com/ArchNets/XrayR-release/master/install.sh)
```
...
docker pull ghcr.io/ArchNets/xrayr:latest && docker run --restart=always --name xrayr -d -v ${PATH_TO_CONFIG}/config.yml:/etc/XrayR/config.yml --network=host ghcr.io/ArchNets/xrayr:latest
```
...
# ดึงการตั้งค่า
git clone https://github.com/ArchNets/XrayR-release
...
ConnetionConfig:
  ...
  UplinkOnly: 2 # ขีดจำกัดเวลาขณะปิดการเชื่อมต่อด้านล่าง, วินาที
  DownlinkOnly: 4 # ขีดจำกัดเวลาขณะปิดการเชื่อมต่อหลังจากปิดการเชื่อมต่อด้านบน, วินาที
  BufferSize: 64 # ขนาดแคชภายในของแต่ละการเชื่อมต่อ, kB
Nodes:
  - PanelType: 'ArchNet' # ประเภทแผง: ArchNet, SSpanel, V2board, NewV2board, PMpanel, Proxypanel, V2RaySocks
    ApiConfig:
      ...
      SpeedLimit: 0 # Mbps, การตั้งค่าท้องถิ่นจะแทนที่การตั้งค่าระยะไกล, 0 หมายถึงปิดใช้งาน
      DeviceLimit: 0 # การตั้งค่าท้องถิ่นจะแทนที่การตั้งค่าระยะไกล, 0 หมายถึงปิดใช้งาน
    ...
    ControllerConfig:
      ...
      DNSType: AsIs # AsIs, UseIP, UseIPv4, UseIPv6, กลยุทธ์ DNS
      EnableProxyProtocol: false # ใช้ได้เฉพาะสำหรับ WebSocket และ TCP
      EnableFallback: false # รองรับเฉพาะสำหรับ Trojan และ Vless
      FallBackConfigs: # รองรับหลายการสำรอง
        - SNI: # TLS SNI (Server Name Indication), ว่างสำหรับใครก็ได้
          Path: # HTTP PATH, ว่างสำหรับใครก็ได้
          Dest: 80 # จำเป็น, จุดหมายของการสำรอง, ตรวจสอบที่ https://xtls.github.io/config/fallback/ สำหรับรายละเอียด
