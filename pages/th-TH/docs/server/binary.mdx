...
ในหน้าต่างคำสั่งของ MariaDB ให้ดำเนินการตามคำสั่งต่อไปนี้เพื่อสร้างฐานข้อมูลใหม่ชื่อ archnet_db และตั้งค่าชุดอักขระเป็น utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

ถัดไป สร้างผู้ใช้ฐานข้อมูลท้องถิ่นและจำกัดสิทธิ์ของผู้ใช้ไปยังฐานข้อมูลที่สร้างขึ้นใหม่ โดยใช้ archnet เป็นชื่อผู้ใช้ และ archnet-password เป็นรหัสผ่านของผู้ใช้
...
ที่อยู่ดาวน์โหลด: `https://github.com/ArchNets/archnet/releases`
...
- แก้ไขไฟล์การกำหนดค่า archnet.yaml
...
Logger: # การกำหนดค่าบันทึก
  FilePath: ./archnet.log # เส้นทางไฟล์บันทึก, ค่าเริ่มต้น: ./archnet.log
...
MySQL:
  ...
  Username: archnet # ชื่อผู้ใช้ MySQL, จำเป็นต้องกรอก
  Password: archnet-password # รหัสผ่าน MySQL, จำเป็นต้องกรอก
  Dbname: archnet_db # ชื่อฐานข้อมูล MySQL, จำเป็นต้องกรอก
...
Administer:
  Email: admin@archnets.com # อีเมลเข้าสู่ระบบแผงควบคุม, ค่าเริ่มต้น: admin@archnets.com
...
2. บริการ systemd:
   ...
   - ชื่อบริการ: archnet
   - ไฟล์กำหนดค่าบริการ: /etc/systemd/system/archnet.service
   - คำสั่งเริ่มบริการ: systemctl start archnet
   - คำสั่งหยุดบริการ: systemctl stop archnet
   - คำสั่งรีสตาร์ทบริการ: systemctl restart archnet
   - คำสั่งตรวจสอบสถานะบริการ: systemctl status archnet
   - ตั้งค่าให้เริ่มอัตโนมัติเมื่อบูต: systemctl enable archnet
...
4. บันทึกบริการ: บันทึกบริการจะถูกส่งออกไปยังไฟล์ /usr/local/bin/archnet.log โดยค่าเริ่มต้น

5. สามารถดูบันทึกบริการได้โดยใช้ `journalctl -u archnet -f`

6. เมื่อไฟล์กำหนดค่าเป็นค่าว่างหรือไม่มีอยู่ บริการจะเริ่มต้นด้วยการกำหนดค่าเริ่มต้น โดยเส้นทางไฟล์กำหนดค่าคือ: `./etc/archnet.yaml` โปรดใช้ `http://ที่อยู่เซิร์ฟเวอร์:8080/init` เพื่อเริ่มต้นการกำหนดค่าระบบ
