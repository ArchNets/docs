...
Calea implicită a fișierului de configurare este: `./etc/archnet.yaml`, iar calea poate fi specificată prin parametrul de pornire `--config`.
...
Logger: # Configurare jurnal
  FilePath: # Calea fișierului de jurnal, implicit: ./archnet.log
...
Administrează:
  Email: # Email-ul de conectare în backend, implicit: admin@archnets.com
...
| ------------------ | ---------------------- | :----------------------------------------- |
| ARCHNET_DB          | Configurare MySQL      | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS       | Configurare Redis      | redis://localhost:6379                     |
