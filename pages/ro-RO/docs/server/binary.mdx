...
În interfața de linie de comandă MariaDB, executați următoarea comandă pentru a crea o nouă bază de date archnet_db și a seta setul de caractere la utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Apoi, creați un utilizator local de bază de date și restricționați permisiunile acestuia la baza de date nou creată, folosind archnet ca nume de utilizator și archnet-password ca parolă a utilizatorului.
...
Adresa de descărcare: `https://github.com/ArchNets/archnet/releases`
...
- Modificarea fișierului de configurare archnet.yaml
...
Logger: # Configurarea jurnalului
  FilePath: ./archnet.log # Calea fișierului jurnal, implicit: ./archnet.log
...
MySQL:
  ...
  Username: archnet # Numele de utilizator MySQL, obligatorie
  Password: archnet-password # Parola MySQL, obligatorie
  Dbname: archnet_db # Numele bazei de date MySQL, obligatorie
...
Administer:
  Email: admin@archnets.com # Email pentru autentificarea în backend, implicit: admin@archnets.com
...
2. Serviciul systemd:
   ...
   - Numele serviciului: archnet
   - Fișierul de configurare al serviciului: /etc/systemd/system/archnet.service
   - Comanda de pornire a serviciului: systemctl start archnet
   - Comanda de oprire a serviciului: systemctl stop archnet
   - Comanda de repornire a serviciului: systemctl restart archnet
   - Comanda de stare a serviciului: systemctl status archnet
   - Activarea pornirii la boot: systemctl enable archnet
...
4. Jurnalul serviciului: jurnalul serviciului este, în mod implicit, redirecționat către fișierul /usr/local/bin/archnet.log

5. Puteți vizualiza jurnalul serviciului cu `journalctl -u archnet -f`

6. Când fișierul de configurare este gol sau inexistent, serviciul va porni cu configurația implicită, iar calea fișierului de configurare este: `./etc/archnet.yaml`,
