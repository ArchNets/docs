...
ArchNet, manuel yapılandırma sürecini basitleştiren web tabanlı bir başlatma desteği sunar.

### Başlatma Adımları

1. **Gerekli yapılandırma dosyalarını oluşturun**: Öncelikle `/app/etc/archnet.yaml` dosyasını manuel olarak oluşturun ve yapılandırın, böylece sonraki yapılandırmalar için hazır hale gelsin.
...
5. **Başlatma butonuna tıklayın**: Yapılandırmayı tamamladıktan sonra "Başlat" butonuna tıklayın, uygulama yapılandırmayı otomatik olarak kaydedecek ve yeniden başlatacaktır.
   ...
   > **Not**: ArchNet varsayılan olarak **8080** portunu kullanır, lütfen bu portun erişilebilir olduğundan emin olun ve gerekirse güvenlik duvarı ayarlarını düzenleyin.

## Docker ile Dağıtım

ArchNet varsayılan olarak **8080** portunu kullanır, lütfen bu portun erişilebilir olduğundan emin olun ve gerekirse güvenlik duvarı ayarlarını düzenleyin.

Web başlatma kullanmıyorsanız, varsayılan yönetici hesabıyla giriş yapabilirsiniz:

- **Kullanıcı adı**: `admin@archnets.com`
- **Şifre**: `password`
...
Docker dağıtımına başlamadan önce, sistemin düzgün çalışmasını sağlamak için önce `/app/etc/archnet.yaml` dosyasını manuel olarak oluşturun ve yapılandırın. Ayrıntılı yapılandırma örnekleri için belgenin [Yapılandırma Örnekleri](#yapılandırma-örnekleri) bölümüne bakın.
...
Docker Compose dağıtımına başlamadan önce, sistemin düzgün çalışmasını sağlamak için önce `/app/etc/archnet.yaml` dosyasını manuel olarak oluşturun ve yapılandırın. Ayrıntılı yapılandırma örnekleri için belgenin [Yapılandırma Örnekleri](#yapılandırma-örnekleri) bölümüne bakın.
...
Aşağıda `archnet.yaml` için yapılandırma örnekleri bulunmaktadır:
...
Logger:
  FilePath: ./archnet.log # Günlük dosyası yolu
  MaxSize: 50
  MaxBackup: 3
  MaxAge: 30
  Compress: true
...
Administer:
  ...
  Email: admin@archnets.com # Yönetici e-posta adresi
```
