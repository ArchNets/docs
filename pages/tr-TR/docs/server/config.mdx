...
Yapılandırma dosyasının varsayılan yolu: `./etc/archnet.yaml` olup, başlatma parametresi `--config` ile yapılandırma dosyası yolu belirtilebilir.
...
Logger: # Günlük yapılandırması
  FilePath: # Günlük dosyası yolu, varsayılan: ./archnet.log
...
Administer:
  Email: # Arka plan giriş e-postası, varsayılan: admin@archnets.com
...
| --------------- | -------------------- | :----------------------------------------- |
| ARCHNET_DB       | MySQL yapılandırması | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS    | Redis yapılandırması | redis://localhost:6379"                    |
