...
# Редактирование конфигурации, пожалуйста, обратитесь к параметрам конфигурации, сохраните и продолжите
...
ConnetionConfig:
  ...
  UplinkOnly: 2 # Ограничение времени, когда соединение с downstream закрыто, секунды
  DownlinkOnly: 4 # Ограничение времени, когда соединение закрыто после закрытия uplink, секунды
  BufferSize: 64 # Размер внутреннего кэша каждого соединения, кБ
Nodes:
  - PanelType: 'ArchNet' # Тип панели: ArchNet, SSpanel, V2board, NewV2board, PMpanel, Proxypanel, V2RaySocks
    ApiConfig:
      ApiHost: 'http://127.0.0.1:667'
      ApiKey: '123'
      NodeID: 41
      NodeType: V2ray # Тип узла: V2ray, Shadowsocks, Trojan
      Timeout: 30 # Тайм-аут для API-запроса
    ...
    ControllerConfig:
      ...
      FallBackConfigs: # Поддержка нескольких резервных копий
        - SNI: # TLS SNI (Server Name Indication), пусто для любого
          ...
          Dest: 80 # Обязательно, назначение резервной копии, проверьте https://xtls.github.io/config/fallback/ для получения подробной информации.
