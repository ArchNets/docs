...
У командному рядку MariaDB виконайте наступну команду, щоб створити нову базу даних archnet_db і встановити набір символів на utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Далі створіть локального користувача бази даних і обмежте його права доступу до новоствореної бази даних, використовуючи archnet як ім'я користувача та archnet-password як пароль користувача.
...
Посилання для завантаження: `https://github.com/ArchNets/archnet/releases`
...
- Змініть файл конфігурації archnet.yaml
...
Logger: # Налаштування журналу
  FilePath: ./archnet.log # Шлях до файлу журналу, за замовчуванням: ./archnet.log
...
MySQL:
  ...
  Username: archnet # Ім'я користувача MySQL, обов'язково
  Password: archnet-password # Пароль MySQL, обов'язково
  Dbname: archnet_db # Назва бази даних MySQL, обов'язково
...
Administer:
  Email: admin@archnets.com # Електронна пошта для входу в адмінку, за замовчуванням: admin@archnets.com
...
2. Служба systemd:
   ...
   - Назва служби: archnet
   - Файл конфігурації служби: /etc/systemd/system/archnet.service
   - Команда запуску служби: systemctl start archnet
   - Команда зупинки служби: systemctl stop archnet
   - Команда перезапуску служби: systemctl restart archnet
   - Команда перевірки статусу служби: systemctl status archnet
   - Автозапуск служби при завантаженні: systemctl enable archnet
...
4. Журнал служби: журнали служби за замовчуванням виводяться у файл /usr/local/bin/archnet.log

5. Ви можете переглянути журнали служби за допомогою `journalctl -u archnet -f`

6. Коли файл конфігурації порожній або не існує, служба буде запущена з конфігурацією за замовчуванням, шлях до файлу конфігурації: `./etc/archnet.yaml`,
