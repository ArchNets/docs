...
設定ファイルのデフォルトパスは:`./etc/archnet.yaml`で、起動パラメータ `--config` を使用して設定ファイルのパスを指定できます。
...
Logger: # ログ設定
  FilePath: # ログファイルのパス, デフォルト: ./archnet.log
...
Administer:
  Email: # バックエンドログインメール, デフォルト: admin@archnets.com
...
| 環境変数     | 設定項目  | 例                                          |
| ------------ | --------- | :------------------------------------------ |
| ARCHNET_DB    | MySQL設定 | root:password\@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS | Redis設定 | redis\://localhost:6379"                    |
