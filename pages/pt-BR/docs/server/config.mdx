...
O caminho padrão do arquivo de configuração é: `./etc/archnet.yaml`, que pode ser especificado através do parâmetro de inicialização `--config`.
...
Logger: # Configuração de log
  FilePath: # Caminho do arquivo de log, padrão: ./archnet.log
...
Administer:
  Email: # E-mail de login do painel administrativo, padrão: admin@archnets.com
...
| -------------------- | --------------------- | :----------------------------------------- |
| ARCHNET_DB            | Configuração do MySQL | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS         | Configuração do Redis | redis://localhost:6379                     |
