...
Im MariaDB-Befehlszeileninterface folgenden Befehl ausführen, um eine neue Datenbank archnet_db zu erstellen und den Zeichensatz auf utf8mb4_unicode_ci zu setzen:

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Anschließend einen lokalen Datenbankbenutzer erstellen und dessen Berechtigungen auf die neu erstellte Datenbank beschränken, wobei archnet als Benutzername und archnet-password als Benutzerpasswort verwendet wird.
...
Download-Link: `https://github.com/ArchNets/archnet/releases`
...
- archnet.yaml Konfigurationsdatei bearbeiten
...
Logger: # Protokollkonfiguration
  FilePath: ./archnet.log # Protokolldateipfad, Standard: ./archnet.log
...
MySQL:
  ...
  Username: archnet # MySQL-Benutzername, erforderlich
  Password: archnet-password # MySQL-Passwort, erforderlich
  Dbname: archnet_db # MySQL-Datenbankname, erforderlich
...
Administer:
  Email: admin@archnets.com # E-Mail für den Backend-Login, Standard: admin@archnets.com
...
2. Systemd-Dienst:
   ...
   - Dienstname: archnet
   - Dienstkonfigurationsdatei: /etc/systemd/system/archnet.service
   - Dienststartbefehl: systemctl start archnet
   - Dienststoppbefehl: systemctl stop archnet
   - Dienstneustartbefehl: systemctl restart archnet
   - Dienststatusbefehl: systemctl status archnet
   - Dienstautostart: systemctl enable archnet
...
4. Dienstprotokolle: Die Dienstprotokolle werden standardmäßig in die Datei /usr/local/bin/archnet.log ausgegeben.

5. Die Dienstprotokolle können mit `journalctl -u archnet -f` eingesehen werden.

6. Wenn die Konfigurationsdatei leer oder nicht vorhanden ist, wird der Dienst mit den Standardkonfigurationen gestartet. Der Pfad zur Konfigurationsdatei lautet: `./etc/archnet.yaml`,
