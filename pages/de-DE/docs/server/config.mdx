...
Der Standardpfad zur Konfigurationsdatei ist: `./etc/archnet.yaml`, der Pfad kann über den Startparameter `--config` angegeben werden.
...
Logger: # Log-Konfiguration
  FilePath: # Pfad zur Logdatei, Standard: ./archnet.log
...
Admin:
  Email: # E-Mail für den Backend-Login, Standard: admin@archnets.com
...
| ----------------- | ------------------- | :----------------------------------------- |
| ARCHNET_DB         | MySQL-Konfiguration | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS      | Redis-Konfiguration | redis://localhost:6379                     |
