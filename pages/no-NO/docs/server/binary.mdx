...
I MariaDB-kommandolinjegrensesnittet, kjør følgende kommando for å opprette en ny database archnet_db, og sett tegnsettet til utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Deretter oppretter du en lokal databasebruker og begrenser brukerens rettigheter til den nyopprettede databasen, med archnet som brukernavn og archnet-password som passord.
...
Nedlastingsadresse: `https://github.com/ArchNets/archnet/releases`
...
- Endre archnet.yaml konfigurasjonsfilen
...
Logger: # Loggkonfigurasjon
  FilePath: ./archnet.log # Loggfilbane, standard: ./archnet.log
...
MySQL:
  ...
  Username: archnet # MySQL-brukernavn, påkrevd
  Password: archnet-password # MySQL-passord, påkrevd
  Dbname: archnet_db # MySQL-databasenavn, påkrevd
...
Administer:
  Email: admin@archnets.com # E-post for innlogging til administrasjon, standard: admin@archnets.com
...
2. systemd-tjeneste:
   ...
   - Tjenestenavn: archnet
   - Tjenestekonfigurasjonsfil: /etc/systemd/system/archnet.service
   - Tjenestestartkommando: systemctl start archnet
   - Tjenestestoppkommando: systemctl stop archnet
   - Tjenestestartkommando: systemctl restart archnet
   - Tjenestestatuskommando: systemctl status archnet
   - Tjeneste ved oppstart: systemctl enable archnet
...
4. Tjenestelogg: Tjenestelogg skrives som standard til /usr/local/bin/archnet.log-filen

5. Du kan se tjenesteloggen med `journalctl -u archnet -f`

6. Når konfigurasjonsfilen er tom eller ikke eksisterer, vil tjenesten starte med standardkonfigurasjonen, konfigurasjonsfilens bane er: `./etc/archnet.yaml`, vennligst initialiser systemkonfigurasjonen via `http://server-adresse:8080/init`
