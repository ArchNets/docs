...
In the MariaDB command line interface, execute the following command to create a new database `archnet_db` and set the character set to `utf8mb4_unicode_ci`:

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Next, create a local database user and restrict that user's permissions to the newly created database, using `archnet` as the username and `archnet-password` as the user password:
...
- Modify the `archnet.yaml` configuration file:
...
Logger: # Log configuration
  FilePath: ./archnet.log # Log file path, default: ./archnet.log
...
MySQL:
  ...
  Username: archnet # MySQL username, required
  Password: archnet-password # MySQL password, required
  Dbname: archnet_db # MySQL database name, required
...
Administer:
  Email: admin@archnets.com # Backend login email, default: admin@archnets.com
...
2. systemd service:
   ...
   - Service name: archnet
   - Service configuration file: `/etc/systemd/system/archnet.service`
   - Service start command: `systemctl start archnet`
   - Service stop command: `systemctl stop archnet`
   - Service restart command: `systemctl restart archnet`
   - Service status command: `systemctl status archnet`
   - Service auto-start on boot: `systemctl enable archnet`
...
4. Service logs: By default, service logs are output to the `/usr/local/bin/archnet.log` file.

5. You can view the service logs using `journalctl -u archnet -f`.

6. When the configuration file is empty or does not exist, the service will start with default configurations. The configuration file path is: `./etc/archnet.yaml`. Please initialize the system configuration via `http://server-address:8080/init`.
