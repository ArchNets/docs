...
Dans l'interface de ligne de commande de MariaDB, exécutez la commande suivante pour créer une nouvelle base de données archnet_db et définir le jeu de caractères sur utf8mb4_unicode_ci

```sql
CREATE DATABASE archnet_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

Ensuite, créez un utilisateur de base de données local et limitez les privilèges de cet utilisateur à la base de données nouvellement créée, en utilisant archnet comme nom d'utilisateur et archnet-password comme mot de passe utilisateur.
...
Adresse de téléchargement : `https://github.com/ArchNets/archnet/releases`
...
- Modifier le fichier de configuration archnet.yaml
...
Logger: # Configuration des journaux
  FilePath: ./archnet.log # Chemin du fichier journal, par défaut : ./archnet.log
...
MySQL:
  ...
  Username: archnet # Nom d'utilisateur MySQL, obligatoire
  Password: archnet-password # Mot de passe MySQL, obligatoire
  Dbname: archnet_db # Nom de la base de données MySQL, obligatoire
...
Administer:
  Email: admin@archnets.com # Email de connexion à l'interface d'administration, par défaut : admin@archnets.com
...
2. Service systemd :
   ...
   - Nom du service : archnet
   - Fichier de configuration du service : /etc/systemd/system/archnet.service
   - Commande de démarrage du service : systemctl start archnet
   - Commande d'arrêt du service : systemctl stop archnet
   - Commande de redémarrage du service : systemctl restart archnet
   - Commande d'état du service : systemctl status archnet
   - Démarrage automatique du service : systemctl enable archnet
...
4. Journaux du service : les journaux du service sont par défaut enregistrés dans le fichier /usr/local/bin/archnet.log

5. Vous pouvez consulter les journaux du service avec `journalctl -u archnet -f`

6. Lorsque le fichier de configuration est vide ou inexistant, le service démarrera avec la configuration par défaut, le chemin du fichier de configuration étant : `./etc/archnet.yaml`,
