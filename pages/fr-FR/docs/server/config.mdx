...
Le chemin par défaut du fichier de configuration est : `./etc/archnet.yaml`, il peut être spécifié via le paramètre de démarrage `--config`.
...
Logger: # Configuration des journaux
  FilePath: # Chemin du fichier journal, par défaut : ./archnet.log
...
Administer:
  Email: # Email de connexion à l'interface d'administration, par défaut : admin@archnets.com
...
| ------------------------ | ------------------------ | :----------------------------------------- |
| ARCHNET_DB                | Configuration MySQL      | root:password@tcp(localhost:3306)/vpnboard |
| ARCHNET_REDIS             | Configuration Redis      | redis://localhost:6379                     |
